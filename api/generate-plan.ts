...
// En un entorno como Vercel, no necesitas importar 'Request' y 'Response' explícitamente a menos que uses un framework específico.
// Esta estructura funcionará con las 'Edge Functions' de Vercel.
...
export async function POST(req: Request) {
    // Manejar la petición OPTIONS para CORS preflight
    if (req.method === 'OPTIONS') {
        return new Response('ok', { headers: corsHeaders });
    }
    
    if (!process.env.API_KEY) {
        return new Response(JSON.stringify({ error: "API_KEY no está configurada en el servidor." }), {
            status: 500,
    ...
    try {
        ...
        const plan = await generatePlanInternal(ai, profile);
        ...
        return new Response(JSON.stringify({ error: "Error interno al generar el plan." }), {
            status: 500,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
    }
...
const generatePlanInternal = async (ai: GoogleGenAI, profile: UserProfile): Promise<TrainingPlan> => {
